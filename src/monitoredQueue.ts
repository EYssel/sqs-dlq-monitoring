  ...
  constructor(scope: Construct, id: string, props: IMonitoredQueueProps) {
    ...
    if (!props.topic) {
      topic.addToResourcePolicy(
        new PolicyStatement({
          sid: 'AllowPublishAlarmsOnlyAndEnforceSSL',
          actions: ['sns:Publish'],
          effect: Effect.DENY,
...
function addSlackNotificationDestination(
  ...
  NagSuppressions.addResourceSuppressions(
    ...
    true, // Apply to child resources (like the execution role)
  );

  topic.addSubscription(new LambdaSubscription(slackListener));

  const logRetention = Stack.of(scope).node.tryFindChild('LogRetentionaae0aa3c5b4d4f87b02d85b201efdd8a');
  if (logRetention) {
    NagSuppressions.addResourceSuppressions(
      logRetention,
      [
        {
          id: 'AwsSolutions-IAM4',
          reason: 'AWSLambdaBasicExecutionRole is required for standard CloudWatch logging by the LogRetention singleton lambda.',
        },
        {
          id: 'AwsSolutions-IAM5',
          reason: 'Wildcard permission is generated by CDK LogRetention singleton lambda to allow writing logs.',
        },
      ],
      true, // Apply to child resources (like the execution role)
    );
